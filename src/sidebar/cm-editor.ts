...
function extensions(options: CodeEditorOptions, onChange: (view: EditorView) => void) {
    // Both modifiers, deliberately. CodeMirror's "Mod" is Cmd on macOS and Ctrl elsewhere, but the
    // textarea this replaces ran on `metaKey || ctrlKey`, so binding Mod alone would silently drop
    // Ctrl+Enter for every Mac user who had been using it.
    const run = () => { options.onRun!(); return true; };
    const runKey = options.onRun
        ? [{ key: "Mod-Enter", run, preventDefault: true }, { key: "Ctrl-Enter", run, preventDefault: true }]
        : [];
    return [
        history(),
        drawSelection(),
