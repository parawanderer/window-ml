...
import {
    FONT_KEY, WRAP_KEY, LINES_KEY, STATS_TOKENS_KEY, STATS_TPS_KEY, OUTMAX_KEY, OUTMAX_DEFAULT, OUTTS_KEY, RESWIN_KEY, RESWIN_PREF_KEY, RESWIN_DEFAULT, resWindowPref, VRAMH_KEY, LANE_HIDDEN_KEY, laneHidden, LANE_SCOPE_KEY, laneScoped, SECTIONS_KEY, laneEnabled, showLane, showModels, LANEH_KEY, laneH, LANE_H_DEFAULT, SNAPDOT_KEY, snapDot, PREDICT_KEY, predictView, FOCUS_KEY, focusMode,
...
function mount(): void {
    ...
    chrome.storage.local.get({ [FONT_KEY]: 1, [WRAP_KEY]: true, [LINES_KEY]: false, [STATS_TOKENS_KEY]: true, [STATS_TPS_KEY]: false, [OUTMAX_KEY]: OUTMAX_DEFAULT, [OUTTS_KEY]: true, [RESWIN_KEY]: 0, [RESWIN_PREF_KEY]: RESWIN_DEFAULT, [VRAMH_KEY]: 0, [LANE_HIDDEN_KEY]: [], [LANE_SCOPE_KEY]: true, [SECTIONS_KEY]: null, [LANEH_KEY]: LANE_H_DEFAULT, [SNAPDOT_KEY]: false, [PREDICT_KEY]: false, [VRAM_PALETTE_KEY]: "", [FOCUS_KEY]: false, [BENCH_OPEN_KEY]: false, [BENCH_DOCK_KEY]: "drawer", [BENCH_H_KEY]: 280, [BENCH_SPLIT_KEY]: 0 }, (d: any) => {
        ...
        // reload (it is where you were), and with none stored the chart opens at the preference — which is
        // what "the default when unset" means, and why the picker can always show a preset.
        if (typeof d[RESWIN_PREF_KEY] === "number" && d[RESWIN_PREF_KEY] > 0) resWindowPref.value = d[RESWIN_PREF_KEY];
        resWindowS.value = typeof d[RESWIN_KEY] === "number" && d[RESWIN_KEY] > 0 ? d[RESWIN_KEY] : resWindowPref.value;
        ...
        snapDot.value = !!d[SNAPDOT_KEY];
        predictView.value = !!d[PREDICT_KEY];
