...
const TIP = {
    ...
    protoStream: "Default OFF. Ask for the streamed chat reply as varint-delimited PROTOBUF (Accept: application/protobuf) rather than OpenAI SSE, which re-sends the id/model/created/choices envelope for every token — measured ~22x fewer bytes on a 220-token reply. Purely opt-in and self-negotiating: a backend that doesn't serve it answers with the usual SSE, so nothing breaks. Skipped for any call carrying tools/toolIds/schema, because the encoder does not emit tool calls yet and would drop them silently. Saves BYTES, not time.",
    ...
    return (
        <div class="settings">
            ...
            {tab === "advanced" ? <>
                ...
                <div class="set-note">Ask for the streamed reply as <b>protobuf</b> instead of OpenAI's SSE. The SSE format re-sends the <code>id</code>, <code>model</code>, <code>created</code> and <code>choices[0].delta</code> wrapper <b>for every token</b> — about 224 bytes of envelope around 5 bytes of text; protobuf sends that once and a token costs a tag, a length and its own bytes. Measured at <b>~22x fewer bytes</b> on a 220-token reply. It is one <code>Accept</code> header, so a backend that doesn't speak it (anything but a patched Ollama) answers with the SSE it always did and nothing changes. <b>Bytes, not speed</b> — worth it over a slow link and not otherwise, which is why it is <b>off by default</b>. A call using <b>tools</b> or a <b>schema</b> keeps SSE regardless: the encoder doesn't carry tool calls yet and would drop them silently.</div>
                <label class="set-check">
