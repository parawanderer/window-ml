...
import type {
    ...
    DerefValue, VisionMemory, RebuildConfig, AnswerMedia, MlAnswer} from "./contract";
import { htmlToMarkdown } from "./html-to-md";
import { mlPipe, PIPE_SYNTAX } from "./text-pipe";
import { truncate, elPath, describeSkeleton, queryAll, selectorError, setPierceClosedShadow, viewportRect, jsonShape, joinShapes, jsonValue, shadowHostReport, clickSelector, elLine, isCurrentPage, typeFromExtension } from "./dom";
import { tableFromDelimited, tableShape, asTable } from "./table-data";
import { isTable } from "./table-brand";
import { makeAnswerFacade, resolveOutputs } from "./answer-set";
...
import { emitDebug, sessionRegistry, agentRegistry, handleRegistry, enterAgentRun, exitAgentRun } from "./bus";
...
import { captureVerify, setCdpEnabled } from "./builtin-tools";
...
import { AgentHandle, DerefText } from "./ml-agent";   // run-control object (createAgent/agent) + page-loop same-origin auto-approve predicates
 ...
 */
const createAgent = function(this: MlApi, opts: AgentOptions = {}): MlAgentHandle {
 ...
 */
const resumeAgent = function(this: MlApi, hash: string): MlAgentHandle {
 ...
 */
const approveOnce = function(this: MlApi): (req: ApprovalRequest) => boolean {
 ...
 */
const _rebuildToolset = function(this: MlApi, rebuild: RebuildConfig): MlTool[] {
    const ml = this;
 ...
 */
const _adoptRun = function(this: MlApi, runId: string, rebuild: RebuildConfig): void {
