...
| `SYNTHETIC=0` | Don't mark this run's requests `synthetic`. Every observe (and bench) run tells the patched Ollama its traffic is generated, so the server keeps it out of what it learns keep-alive and placement from: auto-approvals, scripted follow-ups and a killed browser are harness timing, not use. Set `SYNTHETIC=0` only when a PERSON is really driving the run (`WATCH=1 APPROVE=hold`, clicking the approvals). |
