...
**Shipped as a native app of its own** (Capacitor was the decision of 2026-09-17 and its app shipped until the native
shell reached parity on 2026-09-23; the shell is [`NATIVE_SHELL.md`](NATIVE_SHELL.md) and `mobile/`). The app is a React
Native (Expo) shell around ONE WebView holding this page's transcript, and the same build still runs in a browser, as a
plain page:

- **The native app is the real client.** Its code is the package you build and sign (an update is an install), keys
  live in the platform keystore (iOS Keychain, Android Keystore, through `expo-secure-store`), approvals will reach a
...
- **`ClientPlatform` has a NATIVE adapter** (`nativePlatform`, native-embed.tsx) beside the web adapter: storage and
  keys, the camera, share and downloads, each crossing the bridge to the shell. The core does not know which one it has.
...
What it took, in order: the web entry and web adapter with a `FakeHost`; the native shell and its adapter; then pairing
and `HubHost` over the hub's client library.

**CI builds the native project; nobody's machine has to.** `mobile/app.json` and `mobile/plugins/` are the whole of
the configuration and `mobile/android`/`mobile/ios` are generated by `expo prebuild` and gitignored, so a change to
the chat page is never a change to two native projects. Generating them needs no SDK at all; the SDKs are needed only
to COMPILE what they produce, which is what the `mobile-android` job does on every code change and `mobile-ios` does
nightly (a macOS runner costs an order of magnitude more per minute, and what it catches is a packaging break rather
than a behaviour one).

The reason is not thrift. The moment the suite depends on Android Studio or Xcode, a one-line UI fix needs 40GB of
tooling, and the people who would make that fix stop making it. Local setup is in CONTRIBUTING.md as optional prose
and must never become a prerequisite.
...
6. **The phone app**: the native shell (`mobile/`) and its `ClientPlatform` adapter (never served by the hub), then
