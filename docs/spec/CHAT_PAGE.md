...
**CI builds the native projects; nobody's machine has to.** `capacitor.config.ts` is the whole of the configuration
and `android/`/`ios/` are generated by `cap add` and gitignored, so a change to the chat page is never a change to
two native projects. The scaffolding and the sync (`npm run mobile:android` / `mobile:ios`) need no SDK at all —
they write template files and copy `dist-web/` — and the SDKs are needed only to COMPILE what they produce, which
is what the `mobile-android` job does on every code change and `mobile-ios` does nightly (a macOS runner costs an
order of magnitude more per minute, and what it catches is a packaging break rather than a behaviour one).
