...
const STEPS = [
    { step: 1, token: "a1b2c3", tool: "python_exec" },
    { step: 2, token: "d4e5f6", tool: "exec" },
];
...
test("an output quoted inline is not appended again", () => {
    const prose = "Here is the table ![the table](@tool:a1b2c3:out) — as you can see.";
    const answer = "![the table](@tool:a1b2c3:out)";
...
test("identity is the resolved STEP, so a hex and its tool-name alias are the same output", () => {
    ...
    assert.equal(answerWithoutShown("![x](@tool:python_exec:out)", "![y](@tool:a1b2c3:out)", resolve, isAlias).trim(), "");
});

test("an output NOT quoted inline still gets appended", () => {
    const prose = "I ran two things ![one](@tool:a1b2c3:out).";
    const answer = "![first](@tool:a1b2c3:out) ![second](@tool:d4e5f6:out)";
    const left = answerWithoutShown(answer, prose, resolve, isAlias);
    assert.ok(!left.includes("a1b2c3"), "the quoted one is dropped");
    assert.ok(left.includes("d4e5f6"), "the unquoted one survives — it has nowhere else to be shown");
    assert.deepEqual([...tokenIdsIn(left)], ["d4e5f6"]);
});

test("prose around a dropped citation is preserved, not mangled", () => {
    const answer = "before ![x](@tool:a1b2c3:out) after";
    const out = answerWithoutShown(answer, "![x](@tool:a1b2c3:out)", resolve, isAlias);
...
test("the :in slot is the same output as :out for dedup — one step, one render", () => {
    ...
    const out = answerWithoutShown("![result](@tool:a1b2c3:out)", "![code](@tool:a1b2c3:in)", resolve, isAlias);
