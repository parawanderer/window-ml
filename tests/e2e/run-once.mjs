...
const BUILD_INFO_BASE = await import("../../src/build-info.gen.ts").then((m) => m.BUILD_INFO).catch(() => undefined);
// The diff lives in its own generated module so that bundles which never serve it do not carry it; a harness
// artifact DOES want it, so it is folded back in here.
const BUILD_DIFF = await import("../../src/build-diff.gen.ts").then((m) => m.BUILD_DIFF).catch(() => undefined);
const BUILD_INFO = BUILD_INFO_BASE && { ...BUILD_INFO_BASE, dirtyDiff: BUILD_DIFF };
