...
test("remove by string matches a token ref or exact text", () => {
    const a = new AnswerSet();
    a.add(tok("@tool:35bf1f0:out", "table")); a.add(txt("keep me")); a.add(txt("drop me"));
    assert.equal(a.remove("@tool:35bf1f0:out"), 1, "token removed by ref");
...
test("answerItemFromString: @tool: → token, anything else → text", () => {
    assert.deepEqual(answerItemFromString("@tool:35bf1f0:out"), { kind: "token", ref: "@tool:35bf1f0:out" });
...
test("finalizeAnswer: a DESIGNATED token renders at the bottom (with the model's caption)", () => {
    const set = new AnswerSet();
    set.add(answerItemFromString("@tool:aaaaaa8:out", "per-region totals"));
    // No inline citation in the prose → the designated output stays.
    const md = finalizeAnswer(set, "Here are the numbers.");
    assert.equal(md, "![per-region totals](@tool:aaaaaa8:out)");
});

test("finalizeAnswer: DEDUP — a token cited INLINE in the prose is dropped from the bottom", () => {
    const set = new AnswerSet();
    set.add(answerItemFromString("@tool:aaaaaa8:out", "totals"));
    // The model already expanded @tool:aaaaaa8 inline → don't ALSO append it (redundant).
    const md = finalizeAnswer(set, "The totals are [totals](@tool:aaaaaa8:out).");
...
test("finalizeAnswer: a DESIGNATED token NOT cited inline still renders (only the cited one dedupes)", () => {
    const set = new AnswerSet();
    set.add(answerItemFromString("@tool:aaaaaa8:out", "A"));
    set.add(answerItemFromString("@tool:bbbbbbc:out", "B"));
    const md = finalizeAnswer(set, "See [A](@tool:aaaaaa8:out).");
    assert.equal(md, "![B](@tool:bbbbbbc:out)", "A dedupes (inline), B stays (bottom)");
});
...
test("finalizeAnswer: MULTIPLE designated outputs all render, stacked, each deduped vs inline", () => {
    const set = new AnswerSet();
    set.add(answerItemFromString("@tool:aaaaaa8:out", "sales"));
    set.add(answerItemFromString("@tool:bbbbbbc:out", "regions"));
    set.add({ kind: "text", text: "Summary: two tables." });
    // bbbbbb is inline-cited → only aaaaaa + the text stay at the bottom.
    const md = finalizeAnswer(set, "Regions here [r](@tool:bbbbbbc:out).");
    assert.equal(md, "![sales](@tool:aaaaaa8:out)\n\nSummary: two tables.");
});

test("answerItemFromString: a note becomes the token caption; plain text ignores note", () => {
    assert.deepEqual(answerItemFromString("@tool:aaaaaa8:out", "cap"), { kind: "token", ref: "@tool:aaaaaa8:out", preview: "cap" });
...
test("resolveOutputs: a designated DataFrame → a { kind:'table', columns, rows } 2D matrix", () => {
    const renders = [{ id: "aaaaaa8", tool: "python_exec", render: { type: "python-out", df: { columns: ["Rep", "Total"], rows: [["Gia", 850], ["Kim", 810]] } } }];
    const outs = resolveOutputs("[top reps](@tool:aaaaaa8:out)", "The top rep is Gia.", renders);
    assert.deepEqual(outs, [{ id: "aaaaaa8", tool: "python_exec", kind: "table", columns: ["Rep", "Total"], rows: [["Gia", 850], ["Kim", 810]] }]);
});

test("resolveOutputs: a python dict/list value is PARSED to the real JS object (not a stringified blob)", () => {
    const renders = [{ id: "bbbbbbc", tool: "python_exec", render: { type: "python-out", value: '{"grand": 6260, "regions": ["East", "West"]}' } }];
    const outs = resolveOutputs("[stats](@tool:bbbbbbc:out)", "done", renders);
    assert.deepEqual(outs[0], { id: "bbbbbbc", tool: "python_exec", kind: "value", value: { grand: 6260, regions: ["East", "West"] } });
    // a NON-json value stays a string
    assert.equal(resolveOutputs("[v](@tool:cccccc0:out)", "", [{ id: "cccccc0", tool: "exec", result: "just text" }])[0].value, "just text");
});

test("resolveOutputs: INLINE cites + BOTTOM designations both resolve, in appearance order, deduped", () => {
    const renders = [
        { id: "aaaaaa8", tool: "exec", result: "42" },
        { id: "bbbbbbc", tool: "python_exec", render: { type: "python-out", df: { columns: ["x"], rows: [[1]] } } },
    ];
    // inline cite of aaaaaa in the prose; bbbbbb designated at the bottom.
    const outs = resolveOutputs("[table](@tool:bbbbbbc:out)", "The count is [n](@tool:aaaaaa8:out).", renders);
    assert.deepEqual(outs.map(o => o.id), ["aaaaaa8", "bbbbbbc"], "prose (inline) first, then the bottom; deduped by id");
...
test("resolveOutputs: an ELEMENTS output → serialized item previews (live nodes stay in res.elements)", () => {
    const renders = [{ id: "cccccc0", tool: "exec", render: { type: "elements", items: [{ path: "div#a", text: "A" }, { path: "div#b" }] } }];
    assert.deepEqual(resolveOutputs("[els](@tool:cccccc0:out)", "found", renders)[0],
        { id: "cccccc0", tool: "exec", kind: "elements", items: [{ path: "div#a", text: "A" }, { path: "div#b" }] });
});

test("resolveOutputs: an image → a data URL; code → its text; a hallucinated id is skipped", () => {
    const renders = [
        { id: "dddddd4", tool: "look", render: { type: "look", image: "data:image/png;base64,AAAA" } },
        { id: "eeeeee8", tool: "exec", render: { type: "code", text: "1 + 1", lang: "javascript" } },
    ];
    assert.deepEqual(resolveOutputs("[shot](@tool:dddddd4:out)", "", renders)[0], { id: "dddddd4", tool: "look", kind: "image", dataUrl: "data:image/png;base64,AAAA" });
    assert.deepEqual(resolveOutputs("[src](@tool:eeeeee8:in)", "", renders)[0], { id: "eeeeee8", tool: "exec", kind: "code", text: "1 + 1", lang: "javascript" });
