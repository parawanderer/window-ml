...
//   node scripts/ios.mjs install [--demo] build the page, sync it into the app, prebuild + pods as needed, xcodebuild, install
...
// certificate is needed for a simulator build. The app is `mobile/` (docs/spec/NATIVE_SHELL.md); its Xcode project is
...
const BUILT = "mobile/ios/build/Build/Products/Release-iphonesimulator/windowml.app";
...
/** The app: the page built and synced into it, the native project generated (with the UIScene plugin iOS 27 needs, and
 *  again whenever the app's native inputs change: mobile-prebuild.mjs) and its pods installed, a Release build (the JS
 *  bundled in, no Metro), installed. */
function install() {
    const d = need();
