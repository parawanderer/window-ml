...
// `dist-web/` in and installs the native dependencies. Wanting "make sure it exists, then sync it" out of those two
// is a conditional, and a conditional written as `cap add || true && cap sync` in a package script parses as
// something else: `&&` and `||` are LEFT-ASSOCIATIVE with equal precedence, so a failing web build would be swallowed
