...
// The uncommitted diff goes in a module of its OWN, imported only by `tools.ts`. It used to be a field on
// BUILD_INFO, and the day the chat page started carrying BUILD_INFO (for an export's provenance) it carried a
// whole `git diff` with it — bloating a bundle that never reads it, and tripping the web build's `chrome.*` guard
// whenever the working tree happened to contain that string. Nothing tree-shakes a property off an object.
const DIFF_OUT = join(ROOT, "src", "build-diff.gen.ts");
...
export function writeBuildInfo() {
    ...
    writeFileSync(DIFF_OUT,
        "// GENERATED by scripts/gen-build-info.mjs — do not edit. Regenerated on every build.\n" +
        "/** The exact `git diff HEAD` at build time (capped), or \"\" when the tree was clean. Its own module so that\n" +
        " *  only what serves it (`tools.ts`) carries it into a bundle. */\n" +
        `export const BUILD_DIFF = ${JSON.stringify(dirtyDiff)};\n`);
    return OUT;
}
